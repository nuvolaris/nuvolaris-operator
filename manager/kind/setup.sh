...
# deploy a nuvolaris manager pod under the kube-system
cat $HOME/.kube/config | sed -e "s/server: .*/server: https:\/\/kubernetes.default.svc/" > $HOME/.kube/manager-config
kubectl -n kube-system create configmap nuvolaris-manager-cm --from-file=config=$HOME/.kube/manager-config
